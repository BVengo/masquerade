...
from masquerade.signatures import check_signature
...
def test_supported_signature_matches(
    ...
    assert check_signature(path, extension) is True
...
def test_mismatched_signature_is_rejected(tmp_path: Path) -> None:
    ...
    assert check_signature(path, ".jpg") is False


def test_unsupported_extension_is_undetermined(tmp_path: Path) -> None:
    """Return None for extensions without a signature implementation."""
    path = tmp_path / "file.unknown"
    path.write_bytes(b"anything")

    assert check_signature(path, ".unknown") is None
