...
def test_supported_types_validate(
    ...
    magic_check, detailed_check = check_file(fixture_path)
    assert magic_check is True, f"Magic check failed for: {extension}"
    assert detailed_check is True, f"Detailed check failed for: {extension}"
...
def test_supported_types_reject_invalid(
    ...
    magic_check, detailed_check = check_file(invalid_path)

    # Either magic check is False, or detailed check is False in the case
    # where magic check accepted the invalid file.
    assert magic_check is False or detailed_check is False, (
